...
function buildPrompt(studentName, activities, tone, length, customPrompt, memo) {
    let categoryGoal = "";
    ...
    let memoGuide = memo ? `\n- **[?좎깮??愿李?硫붾え 諛섏쁺 諛?臾몄옣 ?뺤옣]**: ???숈깮????섏뿬 ?좎깮?섏씠 湲곕줉???ㅼ쓬 媛쒕퀎 愿李??뱀꽦 ?댁슜???앷린遺 臾몃㎘??理쒖슦?좎쟻?쇰줈 湲대????곌퀎?섍퀬 援ъ껜?뷀븯???띿꽦?섍쾶 臾몄옣?쇰줈 ?뺤옣???묒꽦?섏떗?쒖삤: "${memo}"` : "";
    ...
    if (activeCategory === 'subject') {
        ...
        `;
        dateRule = "**?좎쭨 怨좎젙 諛??곕룄 ?쒖떆 ?덈? 湲덉? 洹쒖튃**: 諛곗젙???쒕룞 ?댁뿭???좎쭨(?? '(3/16)', '(5/15)')媛 議댁옱??寃쎌슦, 諛섎뱶??臾몄옣 ?댁뿉???대떦 ?쒕룞 紐낆묶 諛붾줈 ?ㅼ뿉 ?쒓났??愿꾪샇 ?좎쭨 ?뺤떇 洹몃?濡쒕? 遺숈뿬???묒꽦?섏떗?쒖삤 (?? '?숆탳??젰?덈갑援먯쑁(3/16)??李몄뿬?섏뿬', '?숆툒?먯튂?뚯쓽(4/10)??李몄뿬?섏뿬'). **留ㅼ슦 以묒슂: ?곕룄(?? '2023.03.03.', '2026.04.20.' ??瑜??꾩쓽濡??곸긽?섏뿬 ?㏓텤?닿굅???좎쭨 ?щ㎎???ㅻⅤ寃?蹂?뺥븯???됱쐞瑜??덈? 湲덉??⑸땲?? ?ㅼ쭅 二쇱뼱吏??'???? ?뺥깭??愿꾪샇留?洹몃?濡??ъ슜?섏뿬 ?묒꽦?섍퀬, ?곕룄 ?뺣낫???덈?濡?異붽??섏? 留덉떗?쒖삤.**";
    }
    ...
    1. 二쇱뼱 諛??숈깮 ?대쫫 ?꾩쟾 諛곗젣:
       - **以묒슂**: 臾몄옣 蹂몃Ц ?띿뿉 ?숈깮???대쫫(?? '[?숈깮?대쫫]', '?대쫫', '?숈깮' ????????踰덈룄 ?ъ슜?섏? 留?寃? 
...
async function generateBatchRecords() {
    ...
    for (let i = 0; i < names.length; i++) {
        const studentName = names[i].name;
        const studentMemo = names[i].memo;
        ...
        const promptText = buildPrompt(studentName, studentActivities, selectedTone, selectedLength, customPrompt, studentMemo);
...
async function regenerateOneRecord(index) {
    ...
    const promptText = buildPrompt(student.name, student.activities, selectedTone, selectedLength, customPrompt, student.memo);
