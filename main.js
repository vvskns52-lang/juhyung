...
function buildPrompt(studentName, activities, tone, length, customPrompt) {
    ...
    7. 異쒕젰 ?뺤떇 諛???웾 ?댁떆?쒓렇 異붿텧: 활동명 및 세부 내용 순차 전개 (일괄 나열 절대 금지)**:
       - **오류 패턴 예시**: "활동A(날짜), 활동B(날짜), 활동C(날짜)에 적극적으로 참여하여 사회적 약자를 이해함..." ➔ **이러한 단순 나열식 작성은 절대 금지합니다.**
       - **올바른 작성 패턴**: 반드시 **'활동A(날짜) + A활동의 구체적 내용 서술 ➔ 이어서 활동B(날짜) + B활동의 구체적 내용 서술'** 과 같이 개별 활동이 나온 즉시 그 활동의 수행 팩트가 뒤따라 나오는 유기적인 순차 연결 방식으로 전개해 주십시오.
    7. 출력 형식 및 역량 해시태그 추출:
    ...
    `;gthGuide = "공백 포함 400자 ~ 480자 내외로 세밀한 활동 내용과 관찰된 변화상까지 풍부하게 작성할 것.";
            break;
        default:
